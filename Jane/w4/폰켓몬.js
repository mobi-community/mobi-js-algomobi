...
// @ notes
// map 구조로 변환하여 key 개수 반환하기
// 이때, key의 개수가 N/2 보다 많을 경우 N/2 개수 반환하기

const solution = (nums) => {
  const newSet = new Set(nums);
  const limit = Math.floor(nums.length / 2);

  return newSet.size > limit ? limit : newSet.size;
};
...
// 처음 풀이 (카테고리가 해시여서 해시 맵을 사용해야 한다고 생각했음)
// 하지만 중복 제거만 필요한데 map 구조를 사용할 필요가 없다는 생각이 들었고, set으로 변경
// const makeMap = (arr) => {
//   const m = new Map();
//   for (let x of arr) {
...
//   const result = [...makeMap(nums).keys()].length;
